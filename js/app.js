(() => {
  ...
  menuInit();
  spollers();
  const headersList = document.querySelectorAll(".sliders__header");
  const notesList = document.querySelectorAll(".sliders__note");
  const indicatorsList = document.querySelectorAll(".sliders__indicator");
  const descriptionsList = document.querySelectorAll(".sliders__description");
  let app_index = 0;
  let interval = 5e3;
  let heightsArr = [];
  let heightMax = null;
  setInterval(() => {
    headersList[app_index].classList.toggle("active");
    notesList[app_index].classList.toggle("active");
    indicatorsList[app_index].classList.toggle("active");
    app_index = (app_index + 1) % headersList.length;
    headersList[app_index].classList.toggle("active");
    notesList[app_index].classList.toggle("active");
    indicatorsList[app_index].classList.toggle("active");
  }, interval);
